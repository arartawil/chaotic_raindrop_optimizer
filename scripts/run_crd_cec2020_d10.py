"""
Run the 12 CRD variants on all 10 CEC2020 functions at D = 10.
Settings: pop=30, max_iter=300, n_runs=30.

Each function runs in its own process so the 10 functions execute in parallel.
Output layout:
    experiments/CRD_CEC2020_D10/<CEC20_F*>/CSV Data, Convergence Curves, Box Plots, Excel Files
"""
...
OUTPUT_ROOT = str(PROJECT_ROOT / f"experiments/CRD_CEC2020_D{DIM}")
...
if __name__ == "__main__":
    ...
    for p in processes:
        p.join()
    print(f"CRD CEC2020 D={DIM} (F1..F{N_FUNC}) done!")
