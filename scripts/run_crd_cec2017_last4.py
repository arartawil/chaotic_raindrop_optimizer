"""
Run the 12 CRD variants on the LAST CEC2017 functions (indices 25..28).
heurilab's CEC2017 has 29 functions (F1, F3..F30 — F2 excluded), so after the
prior batches (0..9, 10..14, 15..19, 20..24) only 4 functions remain.

Settings: pop=30, max_iter=300, n_runs=30, dim=30.

Each function runs in its own process so all 4 functions execute in parallel.
Output layout:
...
N_RUNS = 30
DIM = 30
START_IDX = 25        # picks up after the v3 batch
END_IDX = len(CEC2017_FUNCTIONS)   # 29; runs indices 25..28
...
if __name__ == "__main__":
    ...
    for p in processes:
        p.join()
    print(f"CRD CEC2017 D={DIM} (idx {START_IDX}..{END_IDX - 1}) done!")
